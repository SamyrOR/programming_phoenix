defmodule RumblWeb.VideoControllerTest do
  ...
  test "requires user authenticaion on all actions", %{conn: conn} do
    Enum.each(
      [
  ...
  describe "tests with logged user" do
    ...
    test "creates user video and redirects", %{conn: conn, user: user} do
      ...
      {id_number, _} = Integer.parse(id)
      assert html_response(conn, 200) =~ "Video #{id_number}"
